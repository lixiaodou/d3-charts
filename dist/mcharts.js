...
(function (global, factory) {
  ...
  function judgeLengthType(lenght) {
    var value = 0;
  ...
  var grid = {
    ...
    render: function render() {
      ...
      svg.append('path').attr('d', d).attr('fill', gridOption.backgroundColor).attr('stroke', gridOption.borderColor).attr('stroke-width', judgeLengthType(gridOption.borderWidth).type === 'px' ? gridOption.borderWidth : defaultOptions.borderWidth);
    }
  };
  ...
  zrenderProto.initRenderer = function (dom) {
    ...
    var innerDiv = this._innerDiv = select(dom).append('div').style('width', domWidtn + 'px').style('height', domHeight + 'px').style('overflow', 'hidden').style('position', 'relative');
    this._svg = innerDiv.append('svg').attr('width', domWidtn).attr('height', domHeight).attr('xmlns', 'http://www.w3.org/2000/svg').attr('version', '1.1').attr('baseProfile', 'full').style('user-select', 'none').style('position', 'absolute').style('left', 0).style('top', 0);
  };

  zrenderProto.setOption = function (option) {
    var oThis = this;
    var componentsMap = oThis._componentMap = map$1({});
    var optionMap = oThis._optionsMap = map$1(option);
    var allComponentsMap = map$1(components);
    var gridComponent = null; // 属性中是否包含series属性，如果没有包含直接抛出异常
    ...
    allComponentsMap.keys().forEach(function (componentType) {
      var component = allComponentsMap.get(componentType); // 本次渲染使用到的组件以及默认属性的赋值

      if (optionMap.has(componentType)) {
        // @TODO 需要考虑数组的情况
        componentsMap.set(componentType, component);
        var defaultOptionsMap = component && map$1(component['defaultOptions']);
    ...
    if (!optionMap.has('grid')) {
      ...
      var defaultOptionsMap = gridComponent && map$1(gridComponent['defaultOptions']);
  ...
  zrenderProto.initGrid = function () {
    ...
    console.log(gridComponet, 'gridComponet'); // gridComponet.
  };

  zrenderProto.initComponentsOpt = function () {// var oThis = this;
    // var componentMap = oThis._componentMap;
    // var gridComponet = componentMap[comPre + oThis._id + '_grid'];
